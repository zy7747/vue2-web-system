...
<template>
  <div>
    <!-- 搜索栏 -->
    <Collapse :title="$t('notice.notice')" @reset="resetQueryData" @search="search">
...
export default {
    ...
    return {
      ...
      //查询表单基础参数
       queryParams: [
        ...
        {
          type: 'input',
          label: this.$t('notice.roleName'),//消息名称
          prop: "roleName",
          span: 6,
          attributes: {},
          on: {}
        ...
        {
          type: 'input',
          label: this.$t('notice.roleCode'),//消息内容
          prop: "roleCode",
          span: 6,
          attributes: {},
          on: {}
      ...
      tableColumn: [
        ...
        {
          label: this.$t('system.ordinal'),//序号
          type: 'index',
          width: 55
        },
        {
  ...
  methods: {
    //1.查询
    query(page, size) {
      return this.$service.functional.message
    ...
    if (valid) {
      this.$service.functional.message.saveList([this.formData]).then((res) => {
        if (res.code === 200) {
    ...
    deleteLine(row, index) {
      this.$service.functional.message.delete([row]).then((res) => {
        this.search()
      })
    },
    //通过id获取详情
    detail(id) {
      this.$service.functional.message.detail({ id }).then((res) => {
    ...
    this.$modal
        .confirm('是否删除')
        .then(() => {
           this.$service.functional.message.delete(this.checkList).then((res) => {
             this.search()
            })
        })
        .catch(() => {})
    ...
    //导出
    exportExcel() {
      this.$service.functional.message.export().then((res) => {
        this.$download.excel(res, '消息通知.xls')
      })
    }
  }
}
